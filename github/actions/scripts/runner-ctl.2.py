...
class RunnerController:
    ...
    def fetch_token(self, action="registration"):
        """
        Fetch runner token with priority:
        1. GITHUB_TOKEN (direct token)
        2. Generated via GITHUB_PAT API call
        """
        if self.github_token:
            logger.info(f"Using GITHUB_TOKEN for {action}")
            return self.github_token

        if not self.github_pat:
            logger.error(f"Neither GITHUB_TOKEN nor GITHUB_PAT provided for {action}")
            sys.exit(1)

        api_url = self.get_api_url(action)
        logger.info(f"Fetching {action} token from GitHub API...")
